...
class database_coordinates:
    ...
    def create_database(self, cursor):
        try:
            cursor.execute("CREATE DATABASE {} DEFAULT CHARACTER SET `utf8`".format(self.database))
...
class datas_description:
    def __init__(self, API):
        ...
        for key, values in self.API.items():
            self.req_id_cat = (f"SELECT category_id FROM categories WHERE category = '{key}' LIMIT 1")
            self.json_obj = [json.loads((urllib.request.urlopen(values)).read())][0]['products']
            for i, item in enumerate(self.json_obj) :
                self.product_name = validate_string(item.get("product_name", None))
                self.brands = validate_string(item.get("brands", None))
                self.nutrition_grade_fr = validate_string(item.get("nutrition_grade_fr", None))
                self.stores = validate_string(item.get("stores", None))
                self.image_url = validate_string(item.get("image_url", None))
